---
title: "RefNet"
author: "Emilio Bruna, Auriel Fournier, Matt Boone"
date: "`r Sys.Date()`"
...
Now that the algorithm has attempted to match similar names. A user needs to hand check the matches. Open up the 'authors' output from the previous function. There are two important pieces of information in these files:  
...
When you're done, save this spreadsheet, we'll merge the change records in the next step.
...
Now that we've identified matching authors we will calculate the authors location including their lat and longs for later mapping. We will use the **address_lat_long()** function. It has 2 arguments:  
...
The net plot analysis returns an object that allows the plotting of these complex connections. $plot will plot the output to your plotting console.
