...
address_lat_long <- function(data,  
  ...
  paste.frame<-data[,c('state','university','city','country','postal_code','authorID','address')]
  ...
  for(i in 1:nrow(uniqueAddressdress)){
    
    address<-as.character(uniqueAddressdress$short_address[i])
    ...
    if(!is.na(address)){
      ...
      suppressWarnings(result <- geocode(address, output = "latlona", 
                                         source = "dsk",
                                         messaging = TRUE))
      uniqueAddressdress$lat[i]<-result[1]
      uniqueAddressdress$lon[i]<-result[2]
