...
for(i in unique(authors$authorID)){
  ...
  if(length(authors$authorID[authors$authorID==i])>1){print(paste0('Author ID: ',i,' is duplicated please change in the author file and re run'));break}
...
colnames(master)
data1<-master[,c('authorID','AF','groupID','author_order','address','RP_address','RI','OI','UT','refID')]
