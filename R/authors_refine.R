...
#' @param prelim the `prelim` element from the list outputted by 
#' `authors_clean()`
...
#'   path, to which "_authors.csv" and "_authors_references.csv" will be 
#'   appended and the so what you need to do is take the authors file 
#'   and change the names (no au and af), with addresses, title, and authors
...
authors_refine <- function(review, prelim, sim_score = NULL, filename_root="") {
  ...
  for (i in unique(review$authorID)) {
    ...
    if (length(review$authorID[review$authorID == i]) > 1) {
      print(paste0("Author ID: ", i, " is duplicated please change 
...
prelim$AF[prelim$groupID == 
            p] <- prelim$AF[sort(prelim$authorID[prelim$groupID == p])[1]]
  }
  colnames(prelim)
