...
#' This dataset represents the typical formatted output from \code{authors_georef()}
...
#'   \item{university}{also can be considered institution for non-universities}
...
#'   \item{groupID}{ID field for what name group the author is identied as from authors_clean()}
#'   \item{author_order}{numeric, order of author from jounral article}
#'   \item{address}{address of references pulled from the original raw WOS file}
...
#'   \item{RI}{ResearcherID number, identifier given by web of science only, less common than OrcID}
#'   \item{RP_address}{OrcID, unique identifier for researcher given by https://orcid.org }
#'   \item{department}{department which is nested within university}
#'   \item{UT}{unique identifier to each article, given by WOS}
#'   \item{refID}{unique identifier for each article, given by references_read()}
#' }
"BITR_geocode"