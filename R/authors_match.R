...
authors_match <- function(data){
  ...
  matched.df$squash<-paste(matched.df$last, matched.df$f_i,matched.df$m_i)
  matched.df$merged<-F
  options(warn=2)
  ...
  for (p in unique_groupid) {
    ...
    if (!matched & nrow(n_n1) > 0 & !anyNA(n_n1$ID)) {
      ...
      nnfc <- n_n1$f_c>1
      # if(all(!nnfc)) {nnfc <- rep(T, length(jw_m)) }
      # if(length(choice)>1) { choice <- choice[nnfc]}
      # 
      nninf <- apply(n_n1[choice,c('address','RI','OI','email')],1,function(x)sum(!is.na(x))) +
        as.numeric(n_n1$f_c[choice]>1) + as.numeric(n_n1$m_c[choice]>0) 
      
      if(length(choice)>1){ choice <- choice[nninf==max(nninf)]}
      if(length(choice)>1){ choice <- choice[jw_m[choice]==max(jw_m[choice])][1]}
      jw_m <- jw_m[choice]
#         
...
#n_n<-n_backup
  ...
  c.t<-data.frame(table(subset(n_n,ID!=groupID)$groupID))
  c.t$Var1<-as.numeric(as.character(c.t$Var1))
  n_n$merged<-F
  for(r in  c.t$Var1[c.t$Freq>0]){
    #r<-757
  ...
  for (n in unique_names_over1) {
    ...
    if( length(unique(sub$squash))>1){ n_n$flagged[ n_n$groupID ==n ] <- 1}
  }
  ########
  # Find names that dont match still and flag them for review
  # n_n$flagged <- 0
  # for( t in unique(n_n$groupID)){
  #  # t<-757
  #   sub <- subset(n_n, groupID==t & m_c>0)
  #   sub$squash <- paste0(sub$f_i, sub$m_i)
  #   if( length(unique(sub$squash))>1){ n_n$flagged[ n_n$groupID ==t ] <- 1}
  # }
  # 
  ...
  #  q <- unique_names_over2[1]
    ...
    sc5 <- sum(!is.na(author1$country) & author1$country%in%group2$country) * 2
